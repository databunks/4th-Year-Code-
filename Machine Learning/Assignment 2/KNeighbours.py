...
def KNeighboursRegressorFunction(X,y):

    j_values = list(range(1,50,5))
    k_values = ["auto", "ball_tree", "kd_tree", "brute"]

    KNNRegressor = KNeighborsRegressor()
    parameters = {'algorithm': (*k_values,), 'n_neighbors': j_values, 'n_jobs': [-1]}

    # Mae is domain dependent
    # R is domain independent
    
    clf = GridSearchCV(KNNRegressor, parameters, cv=10, refit="R", return_train_score=True, scoring={"MAE":"neg_mean_absolute_error", "R": "r2"})
    ...
    print(f"Best score for KNN Regressor:\n {clf.best_score_:.3f}\n")

    print(f"Mean r2 for KNN Regressor (training):\n {training_mean_R_value}\n")
    print(f"Std r2 for KNN Regressor (training):\n {training_std_R_value}\n")

    print(f"Mean r2 for KNN Regressor (test):\n {test_mean_R_value}\n")
    print(f"Std r2 for KNN Regressor (test):\n {test_std_R_value}\n")

    print(f"Mean MAE for KNN Regressor (training):\n {training_mean_MAE_value}\n")
    print(f"Std MAE for KNN Regressor (training):\n {training_std_MAE_value}\n")

    print(f"Mean MAE for KNN Regressor (test):\n {test_mean_MAE_value}\n")
    print(f"Std MAE for KNN Regressor (test):\n {test_std_MAE_value}\n")
